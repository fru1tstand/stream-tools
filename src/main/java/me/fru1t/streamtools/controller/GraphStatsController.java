...
public class GraphStatsController
    ...
    public void onUpdate(long now) {
        ...
        if (settings.getTimeAxis().isEnabled()) {
            ...
            if (timeAxisPxBetweenMinor > 0) {
                ...
                if (!settings.getTimeAxis().isFirstMinorIncluded()) {
            ...
            if (timeAxisPxBetweenText > 0) {
                ...
                ctx.setFont(Font.font(
                        ...
                        (settings.getTimeAxis().isTextBold() ? FontWeight.BOLD : FontWeight.NORMAL),
                        (settings.getTimeAxis().isTextItalic()
                                ? FontPosture.ITALIC : FontPosture.REGULAR),
                        settings.getTimeAxis().getTextSize()));
                ctx.setFill(settings.getTimeAxis().getTextColor().getColor());
                if (!settings.getTimeAxis().isFirstTextIncluded()) {
        ...
        // Fixed value axis
        if (settings.getFixedValueAxis().isEnabled()) {
            ...
            if (settings.getFixedValueAxis().getMajorEvery() > 1) {
                ...
                if (!settings.getFixedValueAxis().isFirstMajorIncluded()) {
            ...
            if (settings.getFixedValueAxis().getMinorEvery() > 1) {
                ...
                if (!settings.getFixedValueAxis().isFirstMinorIncluded()) {
                    currentPosition -= pxPerLine;
                }
                while (currentPosition > 0) {
                    ctx.fillRect(0,
            ...
            if (settings.getFixedValueAxis().getTextEvery() > 1) {
                ...
                long currentValue = 0;
                if (!settings.getFixedValueAxis().isFirstTextIncluded()) {
                    currentPosition -= pxPerLine;
    ...
    public void onSettingsChange(GraphStatsSettings settings) {
        ...
        scene.getRoot().setStyle(String.format(ROOT_STYLE, settings.getBackgroundColor().getColorHex()));
    ...
    private String parseTimeText(long ms, String value) {
        return value
                .replace(GraphStatsSettings.TIME_AXIS_VALUE_S, Math.round(ms / 1000.0) + "")
                .replace(GraphStatsSettings.TIME_AXIS_VALUE_MS, ms + "");
    }

    private String parseValueText(long value, String string) {
        return string.replace(GraphStatsSettings.VALUE_AXIS_VALUE, value + "");
    }
}
